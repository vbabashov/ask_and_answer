...
import agents  # This should import the OpenAI agents package, not your local one
...
class OrchestratorAgent:
    ...
    async def chat_response(self, question: str) -> Tuple[str, Optional[str]]:
        ...
        try:
            ...
            # Extract the response text - following your reference pattern
            response_text = ""
            ...
            # Try final_output if available (like in your reference)
            if not response_text and hasattr(result, 'final_output'):
                response_text = str(result.final_output)

            # Try to identify which catalog was selected
            selected_catalog = None
            if response_text and "Selected Catalog:" in response_text:
