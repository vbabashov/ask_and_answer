...
class MultiCatalogSystem:
    ...
    def remove_catalog(self, catalog_name: str) -> str:
        """Remove a catalog from the system."""
        try:
            # Remove from library
            if self.catalog_library.remove_catalog(catalog_name):
                # Remove agent if exists
                if catalog_name in self.catalog_agents:
                    del self.catalog_agents[catalog_name]
                
                # Reinitialize orchestrator
                self.orchestrator._initialize_agent()
                
                return f"✅ Successfully removed catalog: {catalog_name}"
            else:
                return f"❌ Catalog {catalog_name} not found"
        except Exception as e:
            return f"❌ Error removing catalog: {str(e)}"

    def get_system_stats(self) -> dict:
        """Get system statistics."""
        return {
            "total_catalogs": self.catalog_library.get_catalog_count(),
            "active_agents": len(self.catalog_agents),
            "catalogs": list(self.catalog_library.catalogs.keys())
        }