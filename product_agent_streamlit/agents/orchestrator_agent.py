...
from config import AGENT_LLM_NAME
...
class OrchestratorAgent:
    ...
    async def chat_response(self, question: str) -> Tuple[str, Optional[str]]:
        """Get chat response from the orchestrator agent."""
        if not self.agent:
        ...
        try:
            ...
            selected_catalog = self._identify_selected_catalog(response_text)
    ...
    def _identify_selected_catalog(self, response_text: str) -> Optional[str]:
        """Identify which catalog was selected from the response."""
